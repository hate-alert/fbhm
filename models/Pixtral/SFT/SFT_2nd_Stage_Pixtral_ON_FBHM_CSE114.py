#!/usr/bin/env python3
"""
Pixtral SFT training script (Trainer + QLoRA) on FBHM dataset (2nd stage).
- Uniform image resizing to avoid patch count mismatch
- Loss computed only on assistant response
- Optional loading of a pre‑trained LoRA adapter (for 2nd stage)
"""
...
# Base model (Pixtral) and optional pre‑trained adapter for 2nd stage
BASE_MODEL = "mistral-community/pixtral-12b"          # or "mistralai/Pixtral-12B-2409"
ADAPTER_PATH: Optional[str] = "DakshJogchand/pixtral-12b-sft-fhm-15-epochs-earlystop-3"     # e.g. "username/pixtral-12b-sft-mami-15-epochs" – set to None if starting fresh

OUTPUT_DIR = "./pixtral-12b-sft-fhm-then-FBHM-3-epochs"
HF_UPLOAD_NAME = "pbhaskar/pixtral-12b-sft-fhm-then-FBHM-3-epochs"   # HF repo name for upload
HF_TOKEN_2 = ""                     # set to enable upload

HF_TOKEN_1 = ""
login(token=HF_TOKEN_1)

# Training hyperparams (adjust based on GPU memory)
MAX_LENGTH = 4096                          # Pixtral can handle longer, but 2048 is safe
...
# LoRA config (used if ADAPTER_PATH is None, or to extend existing adapter)
LORA_R = 16
LORA_ALPHA = 32
LORA_DROPOUT = 0.1

# System prompt (identical to the one used in Qwen3 script)
...
def upload_to_hf(local_dir: str, repo_id: str, token: str) -> bool:
    try:
        ...
        upload_folder(
            ...
            commit_message="Pixtral-12B fine-tuned on FBHM dataset (QLoRA, 2nd stage)",
            repo_type="model"
        )
...
# Dataset class
# ---------------------------
class PixtralFbhmDataset(torch.utils.data.Dataset):
...
def data_collator(examples: List[Dict], processor: AutoProcessor, target_wh: tuple, max_length: int):
    ...
    # For Mistral-based models, assistant response starts after [/INST]
    marker = "[/INST]"
    ...
    for i, (input_ids, conv) in enumerate(zip(batch_ids, convs)):
        start_idx = -1
        if marker_ids:
            # Find last occurrence of marker (the one right before the assistant answer)
            n = len(marker_ids)
...
def main():
    ...
    if ADAPTER_PATH is not None:
        ...
        model.enable_input_require_grads()
        print("No adapter path provided; creating new LoRA.")
    ...
    # # --- Adapter handling (2nd stage) ---
    # if ADAPTER_PATH is not None:
    #     print(f"Loading pre‑trained adapter from {ADAPTER_PATH} for continued training...")
    #     model = PeftModel.from_pretrained(model, ADAPTER_PATH, is_trainable=True)
    # else:
    #     print("No adapter path provided; creating a new LoRA adapter.")
    #     # LoRA target modules (common for Mistral-based models)
    #     target_modules = [
    #         "q_proj", "k_proj", "v_proj", "o_proj",
    #         "gate_proj", "up_proj", "down_proj"
    #         # optionally add vision encoder modules, e.g. "vision_encoder.*"
    #     ]
    #     lora_config = LoraConfig(
    #         r=LORA_R,
    #         lora_alpha=LORA_ALPHA,
    ...
    # Datasets
    train_dataset = PixtralFbhmDataset(train_samples)
    eval_dataset = PixtralFbhmDataset(val_samples) if val_samples else None
