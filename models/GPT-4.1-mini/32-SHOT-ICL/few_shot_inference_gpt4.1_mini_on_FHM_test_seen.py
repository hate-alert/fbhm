...
OUTPUT_TXT = "./32_shot_gpt4.1mini_inference_on_FHM_test_seen_output.txt"
...
def resolve_fhm_image_path(img_field):
    """Resolve FHM image path for test images."""
    # 'img' field contains filename like "img_12345.jpg"
    img_filename = os.path.basename(img_field)
...
class GPT4MiniInference:
    ...
    def predict(self, sample):
        """
        sample: dict from FHM test_seen with keys: id, img, label, text
        """
        img_path = resolve_fhm_image_path(sample['img'])
        if img_path is None:
            return f"Error: Image not found: {sample['img']}"
