...
OUTPUT_TXT = "./32_shot_gpt4.1mini_inference_on_FBHM_test_output.txt"
...
def main():
    ...
    with open(OUTPUT_TXT, "w", encoding="utf-8") as out_f:
        for idx, meme_row in enumerate(tqdm(test_data, desc="Processing memes")):
            ...
            # out_f.write(f"User: {SYSTEM_PROMPT}\n\n")
            # out_f.write(f"Extracted text: ```{meme_row['text']}```\n")
            # out_f.write("Provide your answer in the format: hateful or not-hateful.\n\n")
